...
def train_bpr_model_with_hard_negative_sampling_vectorize(
    ...
    for epoch in range(num_epochs):
        ...
        for users, pos_items in tqdm(dataloader, desc=f"Epoch {epoch+1}/{num_epochs}"):
            ...
            # neg_items_index = torch.randint(0, num_items, (batch_size, num_candidate_neg_samples), device=device)
            neg_items_index = []
            ...
            users_expanded = users.unsqueeze(1).expand(-1, num_candidate_neg_samples) 

            
            with torch.no_grad():
                users_expanded_flatten = users_expanded.reshape(-1) #(B*M,)
                neg_items_index_flatten = neg_items_index.reshape(-1)#(B*M,)
                candidate_neg_scores = model(users_expanded_flatten,neg_items_index_flatten).view(batch_size, num_candidate_neg_samples) 

            actual_k_hard = min(num_hard_neg_samples, num_candidate_neg_samples)
            _, hard_indices_in_candidates = torch.topk(candidate_neg_scores, k=actual_k_hard, dim=1) 
            hard_neg_items = torch.gather(neg_items_index, 1, hard_indices_in_candidates) 
            ...
            if num_random_neg_samples > 0:
                ...
                #random_neg_items = torch.gather(neg_items_index, 1, random_selection_index)
                final_selected_neg_items = torch.cat([hard_neg_items, random_selection_index], dim=1) 
            else:
            ...
            pos_scores_expanded = pos_scores.unsqueeze(1).expand(-1, num_selected_negs_per_pos) 
        ...
        if training_time >= stopping_time:
            print(f"Out of Time Limit {stopping_time}s at epoch {epoch}, Best Val MAP@{top_k}: {best_map:.4f}")
            break
        else:
            print(f"training_time: {training_time}s , limit: {stopping_time}s")
        
    model.load_state_dict(best_state)
    plot_train_val(training_losses,val_maps,num_epochs,top_k,save_path="train_history_bpr_hard_sample.png")
    torch.save(model.state_dict(), "best_bpr_model.pth")
    print(f"Saving Best model at best_bpr_model.pth,  Best Val MAP@{top_k}: {best_map:.4f}")
...
def train_bpr_model_with_hard_negative_sampling(
    ...
    for epoch in range(num_epochs):
        ...
        for users, pos_items in tqdm(dataloader, desc=f"Epoch {epoch+1}/{num_epochs}"):
            ...
            with torch.no_grad():
                for i in range(batch_size):
                    ...
                    while len(current_candidate_neg_items) < num_candidate_neg_samples  and attempts < num_candidate_neg_samples* 5:
                        ...
                        if sampled_item_idx not in user_known_positives: 
                    ...
                    user_tensor_expanded = users[i].unsqueeze(0).expand(len(current_candidate_neg_items)) 
                    ...
                    if len(final_neg_for_user) == 0 and len(current_candidate_neg_items) > 0: 
                        final_neg_for_user = candidate_neg_items_tensor[0:1] 
            ...
            pos_scores_bpr = model(users, pos_items) 
            ...
            if not expanded_pos_scores: 
                loss_bpr = torch.tensor(0.0, device=device, requires_grad=True) 
            else:
                final_pos_scores = torch.cat(expanded_pos_scores) 
                final_neg_scores = torch.cat(all_selected_neg_scores) 
...
def train_bce_model(model, train_data,train_pos_dict,val_pos_dict,num_items, train_config):
    ...
    model.load_state_dict(best_state)
    plot_train_val(training_losses,val_maps,num_epochs,top_k,save_path="training_history_bce.png")

    
    torch.save(model.state_dict(), "best_bce_model.pth")
    print(f"Saving Best model at best_bce_model.pth,  Best Val MAP@{top_k}: {best_map:.4f}")
...
def train_bpr_model(model, train_data,train_pos_dict,val_pos_dict,num_items, train_config):
    ...
    model.load_state_dict(best_state)
    plot_train_val(training_losses,val_maps,num_epochs,top_k,save_path="training_history_bpr.png")
...
def plot_train_val(train_losses, val_score, epochs,top_k ,save_path='training_history.png'):

    
    fig, axes = plt.subplots(1, 2, figsize=(12, 5))
    axes[0].plot( train_losses, marker='o', linestyle='-')
    axes[0].set_title('Training Loss')
    axes[0].set_xlabel('Epoch')
    axes[0].set_ylabel('Loss')
    axes[0].grid(True)
    axes[1].plot( val_score, marker='o', linestyle='-')
    axes[1].set_title(f'Validation MAP@{top_k}')
